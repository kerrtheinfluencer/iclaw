...
// WASM runs on main thread via WasmRunner.jsx — worker just signals ready
// Safari iOS blocks dynamic import() in workers, so we bypass worker entirely
engineType = ‘wasm’;
report(‘status’, { status: ‘ready’, message: ‘WASM: use model picker to load offline model.’ });
}

async function inferWASM(messages) {
// WASM inference is handled by WasmRunner.jsx on the main thread
// This should never be called — App.jsx intercepts wasm engine calls
throw new Error(‘WASM inference handled on main thread — this should not be called’);
}
